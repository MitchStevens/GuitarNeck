...
import DOM.Event.MouseEvent (clientX)
...
paint_number :: Tuple Context2D NeckData -> Int -> forall e. EffM e Unit
...
paint_chord :: NeckData -> Fingering -> forall e. EffM e Unit
...
paint_centeroids :: forall f e. Foldable f => NeckData -> f Point -> EffM e Unit
paint_centeroids neck points = void $ unsafePartial do
  Just canvas <- getCanvasElementById "guitar_notes"
  ctx <- getContext2D canvas
  setGlobalAlpha ctx 0.35
    *> setFillStyle chord_color ctx
    *> traverse_ (paint_single ctx) points
    *> setGlobalAlpha ctx 1.0
  where
    paint_single :: Context2D -> Point -> EffM e Unit
    paint_single ctx (Point p) = void $
      beginPath ctx
        >>= flip arc (circle { x: p.x, y: p.y, r: radius })
        >>= fill
